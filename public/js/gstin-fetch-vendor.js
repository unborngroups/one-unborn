// GSTIN by PAN Fetch Script for Vendor Master with Multiple Selection
...
function fillPrimaryGSTINDetails() {
    ...
    // Fill Business Name (vendor_name field)
    if (primaryGstin.trade_name && document.getElementById("vendor_name")) {
        document.getElementById("vendor_name").value = primaryGstin.trade_name;
    }
...
function saveSelectedGSTINs() {
    ...
    // Get vendor_id from form or URL
    ...
    if (!vendorId || isNaN(vendorId)) {
        alert('Please save the vendor first, then select GSTINs');
        return;
    }
    ...
    fetch(window.location.origin + '/vendors/save-selected-gstins', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
