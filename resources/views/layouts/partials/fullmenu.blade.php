...
<div class="sidebar bg-dark text-white vh-100 overflow-auto" style="width: 250px; position: fixed; top: 0; left: 0;">
    <div class="p-3">
        ...
        <ul class="nav flex-column">
                    ...
                    <div class="collapse {{ request()->is('companies*') || request()->is('users*') || request()->is('usertypetable*') || request()->is('clients*') || request()->is('vendors*') ? 'show' : '' }}" id="masterMenu">
                        <ul class="nav flex-column ms-3 mt-2">
                            ...
                            @if($users && $users->can_menu)
                                <li><a class="nav-link text-white menu-item {{ request()->is('users*') ? 'active' : '' }}" href="{{ route('users.index') }}"><i class="bi bi-people"></i> Manage Users</a></li>
                            @endif
            ...
            @endphp
            @if($feasibility && $feasibility->can_menu)
                <li class="nav-item">
                    <a class="nav-link text-white d-flex justify-content-between align-items-center"
                    ...
                    <div class="collapse {{ request()->is('feasibility*') ? 'show' : '' }}" id="operationsMenu">
                        <ul class="nav flex-column ms-3 mt-2">
                            <li><a class="nav-link text-white menu-item {{ request()->is('feasibility*') ? 'active' : '' }}" href="{{ route('feasibility.index') }}"><i class="bi bi-diagram-3"></i> Feasibility</a></li>
                           <li><a class="nav-link text-white menu-item {{ request()->is('feasibility-status/open') ? 'active' : '' }}" 
                          href="{{ route('feasibility.status.index', ['status' => 'Open']) }}">
                        <i class="bi bi-hourglass-split"></i> Open
                       </a></li>

                          <li><a class="nav-link text-white menu-item {{ request()->is('feasibility-status/inprogress') ? 'active' : '' }}" 
                                 href="{{ route('feasibility.status.index', ['status' => 'InProgress']) }}">
                             <i class="bi bi-clock-history"></i>  In Progress
                          </a></li>

                          <li><a class="nav-link text-white menu-item {{ request()->is('feasibility-status/closed') ? 'active' : '' }}" 
                                href="{{ route('feasibility.status.index', ['status' => 'Closed']) }}">
