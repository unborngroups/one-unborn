...
<h3 class="mb-3">Asset List</h3>
    ...
    @if($permissions->can_add)
        ...
        <a href="{{ route('operations.asset.create') }}" class="btn btn-success float-end mb-3">
        ...
        <div class="card-header bg-light d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex align-items-center gap-2">
                @if($permissions->can_delete)
                    <form id="bulkDeleteForm" action="{{ route('operations.asset.bulk-delete') }}" method="POST" class="d-inline">
                        @csrf
                ...
                @if($permissions->can_view)
                    <form id="bulkPrintForm" action="{{ route('operations.asset.bulk-print') }}" method="POST" target="_blank" class="d-inline">
                        @csrf
...
<tr>
    ...
    <a href="{{ route('operations.asset.edit', $a->id) }}" class="btn btn-sm btn-primary">Edit</a>
    @endif

        <!-- delete action -->
    @if ($permissions->can_delete)
            <form action="{{ route('operations.asset.destroy', $a->id) }}" method="POST" style="display:inline-block;">
    @csrf
    @method('DELETE')
 ...
 {{-- View --}}
                                   ...
                                   <a href="{{ route('operations.asset.view', $a->id) }}" class="btn btn-sm btn-warning">
...
<p>{{ $a->asset_id }}</p>
    ...
    <!-- print button -->
    <td>
        <a href="{{ route('operations.asset.print', $a->id) }}" class="btn btn-dark" target="_blank">
...
<div class="d-flex justify-content-between align-items-center mt-2 flex-wrap">
            <div class="text-muted small">
                ...
                {{ $assets->lastItem() ?? 0 }}
                of
                {{ number_format($assets->total()) }} entries
            </div>
            <div class="ms-auto">
                <nav>
                    <ul class="pagination mb-0">
                        ...
                        @else
                            <li class="page-item"><a class="page-link" href="{{ $assets->previousPageUrl() }}" rel="prev">Previous</a></li>
                        @endif
                        ...
                        @if ($start > 1)
                            <li class="page-item"><a class="page-link" href="{{ $assets->url(1) }}">1</a></li>
                            @if ($start > 2)
                        ...
                        @for ($i = $start; $i <= $end; $i++)
                            ...
                            @else
                                <li class="page-item"><a class="page-link" href="{{ $assets->url($i) }}">{{ $i }}</a></li>
                            @endif
                        @endfor
                        ...
                        @if ($end < $total)
                            @if ($end < $total - 1)
                                <li class="page-item disabled"><span class="page-link">...</span></li>
                            @endif
                            <li class="page-item"><a class="page-link" href="{{ $assets->url($total) }}">{{ $total }}</a></li>
                        @endif

                        {{-- Next Page Link --}}
                        @if ($assets->hasMorePages())
                            <li class="page-item"><a class="page-link" href="{{ $assets->nextPageUrl() }}" rel="next">Next</a></li>
                        @else
