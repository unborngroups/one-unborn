...
<div class="container mt-4">
    ...
    <tr>
        ...
        <!-- RIGHT SIDE -->
        <td style="width:50%; text-align: left;">
            <strong>Place Of Supply</strong> : {{ $feasibility->vendor->state ?? '' }} <br>
            <strong>Service ID</strong> : {{ $purchase->service_id ?? '' }} <br>
            <strong>UNBORN Service ID/Circuit_id</strong> : {{ $purchase->service_id ?? '' }} | {{ $deliverablePlan->circuit_id ?? '' }} <br>
    ...
    <tr>
        <td class="Bill">
            <strong>{{ $feasibility->vendor->vendor_name ?? '' }}</strong><br>
            {{ $feasibility->vendor->vendor_name ?? '' }}<br>
            {{ $feasibility->vendor->address1 ?? '' }}<br>
            {{ $feasibility->vendor->city ?? '' }},
            {{ $feasibility->vendor->state ?? '' }} -
            {{ $feasibility->vendor->pincode ?? '' }}<br>
            GSTIN: {{ $feasibility->vendor->gstin ?? '' }}
        </td>
