...
function barcode128($text) {
    ...
    $imgWidth = strlen($pattern) * $barWidth + ($quietZone * 2);
    $img = imagecreate($imgWidth, $barHeight);
    ...
    for ($i = 0, $len = strlen($pattern); $i < $len; $i++) {
        if ($pattern[$i] === '1') {
            imagefilledrectangle($img, $x, 0, $x + $barWidth - 1, $barHeight, $black);
        }
        $x += $barWidth;
    }
