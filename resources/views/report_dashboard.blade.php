...
<div class="container-fluid">
            ...
            <form method="get" id="dashboard-filter-form" class="g-3">
                <div class="row align-items-end">
                    <div class="col-md-2">
                        ...
                        <input type="date" name="filter_date" id="filter_date" value="{{ $filterDate ?? '' }}" class="form-control form-control-sm" onchange="this.form.submit()">
                    </div>
                    <div class="col-md-2">
                        <label for="filter_month" class="form-label fw-semibold">Month</label>
                        <input type="month" name="filter_month" id="filter_month" value="{{ $filterMonth ?? '' }}" class="form-control form-control-sm" onchange="this.form.submit()">
                    </div>
                    <div class="col-md-2">
                        <label for="filter_day" class="form-label fw-semibold">Day(s)</label>
                        <input type="number" min="1" max="31" name="filter_day" id="filter_day" value="{{ $filterDay ?? '' }}" class="form-control form-control-sm" placeholder="e.g. 6" onchange="this.form.submit()">
                    </div>
                    <div class="col-md-2">
                        <label for="filter_year" class="form-label fw-semibold">Year</label>
                        <input type="number" min="2000" max="2100" name="filter_year" id="filter_year" value="{{ $filterYear ?? '' }}" class="form-control form-control-sm" placeholder="e.g. 2026" onchange="this.form.submit()">
                    </div>
                    <div class="col-md-2">
    ...
    <div class="row g-4 mb-4">
        <div class="col-12">
            <div class="section-header">
                ...
                <span class="badge bg-primary">{{ array_sum($feasibilityCounts) }} Total</span>
            </div>
        </div>
        <div class="col-md-4">
            <div class="dashboard-card" style="background:linear-gradient(135deg, #28a745, #20c997);" onclick="showTable('feasibility','open')">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <h3 class="dashboard-count">{{ $feasibilityCounts['open'] }}</h3>
                ...
                <div class="update-info">
                    <i class="bi bi-clock me-1"></i> {{ optional($feasibilityUpdates['open']->updatedUser)->name ?? '-' }} • {{ $feasibilityUpdates['open']->updated_at->diffForHumans() }}
                </div>
                @endif
            </div>
        </div>
        <div class="col-md-4">
            <div class="dashboard-card" style="background:linear-gradient(135deg, #fd7e14, #ff922b);" onclick="showTable('feasibility','inprogress')">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <h3 class="dashboard-count">{{ $feasibilityCounts['inprogress'] }}</h3>
                ...
                <div class="update-info">
                    <i class="bi bi-clock me-1"></i> {{ optional($feasibilityUpdates['inprogress']->updatedUser)->name ?? '-' }} • {{ $feasibilityUpdates['inprogress']->updated_at->diffForHumans() }}
                </div>
                @endif
            </div>
        </div>
        <div class="col-md-4">
            <div class="dashboard-card" style="background:linear-gradient(135deg, #17a2b8, #20c997);" onclick="showTable('feasibility','closed')">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <h3 class="dashboard-count">{{ $feasibilityCounts['closed'] }}</h3>
                ...
                <div class="update-info">
                    <i class="bi bi-clock me-1"></i> {{ optional($feasibilityUpdates['closed']->updatedUser)->name ?? '-' }} • {{ $feasibilityUpdates['closed']->updated_at->diffForHumans() }}
                </div>
                @endif
            </div>
        </div>
    ...
    <div class="row g-4 mb-4">
        <div class="col-12">
            <div class="section-header">
                ...
                <span class="badge bg-primary">{{ array_sum($poCounts) }} Total</span>
            </div>
        </div>
        <div class="col-md-4">
            <div class="dashboard-card" style="background:linear-gradient(135deg, #6f42c1, #9b59b6);" onclick="showTable('po','open')">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <h3 class="dashboard-count">{{ $poCounts['open'] }}</h3>
                        <h4 class="dashboard-label">Open</h4>
                    </div>
                ...
                @if($poUpdates['open'])
                <div class="update-info">
                    <i class="bi bi-clock me-1"></i> {{ optional($poUpdates['open']->updatedUser)->name ?? '-' }} • {{ $poUpdates['open']->updated_at->diffForHumans() }}
                </div>
                @endif
            </div>
        </div>
        <div class="col-md-4">
            <div class="dashboard-card" style="background:linear-gradient(135deg, #16a085, #20c997);" onclick="showTable('po','closed')">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <h3 class="dashboard-count">{{ $poCounts['closed'] }}</h3>
                        <h4 class="dashboard-label">Closed</h4>
                    </div>
                ...
                @if($poUpdates['closed'])
                <div class="update-info">
                    <i class="bi bi-clock me-1"></i> {{ optional($poUpdates['closed']->updatedUser)->name ?? '-' }} • {{ $poUpdates['closed']->updated_at->diffForHumans() }}
                </div>
                @endif
            </div>
        </div>
    ...
    <div class="row g- mb-4">
        <div class="col-12">
            <div class="section-header">
                ...
                <span class="badge bg-primary">{{ array_sum($deliverableCounts) }} Total</span>
            </div>
        </div>
        <div class="col-md-4">
            <div class="dashboard-card" style="background:linear-gradient(135deg, #dc3545, #ff4e50);" onclick="showTable('deliverable','open')">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <h3 class="dashboard-count">{{ $deliverableCounts['open'] }}</h3>
                        <h4 class="dashboard-label">Open</h4>
                    </div>
                ...
                <div class="update-info">
                    <i class="bi bi-clock me-1"></i> {{ optional($deliverableUpdates['open']->updatedUser)->name ?? '-' }} • {{ $deliverableUpdates['open']->updated_at->diffForHumans() }}
                </div>
                @endif
            </div>
        </div>
        <div class="col-md-4">
            <div class="dashboard-card" style="background:linear-gradient(135deg, #fd7e14, #fc913a);" onclick="showTable('deliverable','inprogress')">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <h3 class="dashboard-count">{{ $deliverableCounts['inprogress'] }}</h3>
                        <h4 class="dashboard-label">In Progress</h4>
                    </div>
                    <i class="bi bi-arrow-repeat dashboard-icon-bg"></i>
                </div>
                ...
                <div class="update-info">
                    <i class="bi bi-clock me-1"></i> {{ optional($deliverableUpdates['inprogress']->updatedUser)->name ?? '-' }} • {{ $deliverableUpdates['inprogress']->updated_at->diffForHumans() }}
                </div>
                @endif
            </div>
        </div>
        <div class="col-md-4">
            <div class="dashboard-card" style="background:linear-gradient(135deg, #28a745, #28c76f);" onclick="showTable('deliverable','delivered')">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <h3 class="dashboard-count">{{ $deliverableCounts['delivered'] }}</h3>
                ...
                <div class="update-info">
                    <i class="bi bi-clock me-1"></i> {{ optional($deliverableUpdates['delivered']->updatedUser)->name ?? '-' }} • {{ $deliverableUpdates['delivered']->updated_at->diffForHumans() }}
                </div>
                @endif
            </div>
        </div>
    ...
    <script>
        ...
        function downloadSelectedExcel() {
            ...
            const url = `{{ route('report_dashboard.downloadExcel') }}`;
